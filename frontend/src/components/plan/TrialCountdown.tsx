...
export function TrialCountdown({ planExpiresAt, status, size = "md" }: TrialCountdownProps) {
  ...
  if (status === "TRIAL" && planExpiresAt) {
    ...
    return (
      ...
      >
        <svg
          className={`shrink-0 ${expired ? "text-red-500" : "text-emerald-500"}`}
          width={iconSize[size]}
  ...
  if (status === "ACTIVE" && planExpiresAt) {
    ...
    return (
      <div className={`inline-flex items-center gap-1.5 font-medium rounded-xl border bg-blue-50 text-blue-700 border-blue-200 ${sizeClasses[size]}`}>
        <svg
  ...
  if (status === "SUSPENDED") {
    return (
      <div className={`inline-flex items-center gap-1.5 font-medium rounded-xl border bg-amber-50 text-amber-700 border-amber-200 ${sizeClasses[size]}`}>
        <svg
  ...
  if (status === "CANCELLED") {
    return (
      <div className={`inline-flex items-center gap-1.5 font-medium rounded-xl border bg-red-50 text-red-700 border-red-200 ${sizeClasses[size]}`}>
        <svg
