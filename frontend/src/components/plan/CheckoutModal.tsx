...
export function CheckoutModal({ isOpen, onClose, onSuccess }: CheckoutModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in">
      <dialog
        className="relative z-10 w-full p-6 mx-auto max-w-2xl rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#111113] shadow-2xl animate-in zoom-in-95 transition-colors"
        open
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-slate-800 dark:text-white">Ativar plano</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              className={`flex items-center justify-center gap-2 p-4 rounded-2xl border-2 font-bold text-sm transition-all cursor-pointer ${
                selectedMethod === "PIX"
          ...
          <div className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-2xl p-4 text-sm text-slate-600 dark:text-slate-300">
            {selectedMethod === "PIX" ? (
