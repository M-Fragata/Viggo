    ...
    <>
      <div className="sm:hidden space-y-3">
        {data.map((checkin) => {
          ...
          return (
            <div key={checkin.employeeId} className="border border-slate-200 rounded-2xl overflow-hidden">
              <div
                onClick={() => setExpandedId(isExpanded ? null : checkin.employeeId)}
                className="flex items-center justify-between p-4 cursor-pointer hover:bg-slate-50 transition-colors"
              >
                <span className="font-semibold text-slate-800">{checkin.employeeName}</span>
      ...
      <div className="hidden sm:block overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[900px]">
          ...
          <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
            {data.map((checkin) => (
              <tr key={checkin.employeeId} className="hover:bg-slate-50/50 transition-colors">
                <td className="p-4 font-semibold text-slate-800">{checkin.employeeName}</td>
                <td className="p-4">
                  {checkin.entrada ? (
...
function CheckinField({ label, entry }: { label: string; entry?: { timestamp: string; lat: number | null; lng: number | null } }) {
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <span className="text-sm font-medium text-slate-700 flex-1 text-center">{entry ? formatTime(entry.timestamp) : "--"}</span>
      {entry?.lat && entry?.lng ? (
        <a
          href={getGoogleMapsUrl(entry.lat, entry.lng)}
          target="_blank"
          rel="noopener noreferrer"
          className="text-slate-300 hover:text-emerald-500 transition-colors w-6 flex justify-end"
