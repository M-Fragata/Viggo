...
export function AuthLayout({
  ...
  return (
    <div className="min-h-screen bg-black text-on-dark flex flex-col justify-between selection:bg-brand-green selection:text-black relative overflow-x-hidden font-sans">
      {/* Ambient background glows */}
      <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-brand-green/10 blur-[140px] pointer-events-none -z-0" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full bg-emerald-600/10 blur-[160px] pointer-events-none -z-0" />

      {/* Top Header - Renderizado condicionalmente */}
      {shouldShowHeader ? (
        <header className="relative z-20 w-full max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between animate-in fade-in duration-300">
          ...
          <Link
            to="/page"
            className="inline-flex items-center gap-2 text-xs font-medium text-on-dark-muted hover:text-brand-green transition-all duration-200 bg-white/[0.03] hover:bg-white/[0.08] px-3.5 py-2 rounded-xl border border-white/10"
          >
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center p-4 sm:p-6 lg:p-8 my-auto w-full">
        <div className="w-full max-w-5xl rounded-[28px] overflow-hidden border border-white/10 shadow-2xl shadow-black/80 grid grid-cols-1 md:grid-cols-12 bg-[#121214] min-h-[560px]">
          ...
          {/* Form Panel */}
          <div
            className={`w-full md:col-span-7 p-6 sm:p-8 lg:p-12 flex flex-col justify-center bg-[#141416]/95 backdrop-blur-xl ${
              isPanelOnLeft ? "order-2" : "order-2 md:order-1"
            }`}
          >
            <div className="w-full space-y-6">
              <header className="text-center space-y-1.5">
                <h1 className="text-2xl sm:text-3xl font-black text-on-dark tracking-tight">
                  {formTitle}
                </h1>
                {formSubtitle && (
                  <p className="text-xs sm:text-sm text-on-dark-muted">
                    {formSubtitle}
                  </p>
                )}
              </header>
      ...
      {/* Footer */}
      <footer className="relative z-20 w-full max-w-6xl mx-auto px-4 sm:px-6 py-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] text-on-dark-muted">
