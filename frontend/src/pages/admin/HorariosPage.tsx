  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      <PageHeader
        ...
        subtitle="Configuração de jornadas de trabalho e tolerâncias"
        helpText="Configure as jornadas de trabalho padrão da empresa (ex: 44h semanais, 12x36, comercial), intervalos obrigatórios e tolerâncias de atraso."
      />
      <WorkScheduleManager />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-[#111113] border border-slate-200 dark:border-white/10 rounded-3xl shadow-sm p-6 transition-colors">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-lg font-bold text-slate-800 dark:text-white">Horários de Trabalho</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm">Crie horários e atribua aos funcionários para aplicação de tolerância CLT</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-2xl p-6 mb-6 space-y-4">
            <h3 className="font-bold text-slate-800 dark:text-white">{editingId ? "Editar Horário" : "Novo Horário"}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                  ...
                  className="w-full px-4 py-2.5 border border-slate-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white dark:bg-white/5 text-slate-800 dark:text-white text-sm" />
              </div>
              <div>
            ...
            <div>
              ...
              {form.jornadaTipo !== "12x36" && <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Auto-preenchido pela escala, mas editável</p>}
            </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {schedules.map((s) => (
              <div key={s.id} className="border border-slate-200 dark:border-white/10 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 dark:bg-white/[0.02]">
                ...
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => handleEdit(s)}
                    className="px-3 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 rounded-lg transition-colors cursor-pointer">
                    Editar
                  </button>
                  <button onClick={() => handleDelete(s.id)}
                    className="px-3 py-1.5 text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-lg transition-colors cursor-pointer">
                    Remover
                  </button>
                </div>
              </div>
