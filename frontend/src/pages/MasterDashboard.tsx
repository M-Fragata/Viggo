...
import { Building2, TrendingUp, TrendingDown, DollarSign, Clock } from "lucide-react";
  ...
  const cards = [
    {
      ...
      icon: Building2,
      color: "blue",
      change: metrics?.growth?.thisMonth,
    },
    {
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-6">
          ...
          <div className="space-y-3">
            {Object.entries(metrics?.planDistribution ?? {}).map(([plan, count]) => (
              <PlanDistributionRow key={plan} plan={plan} count={count as number} total={metrics?.totalCompanies ?? 1} />
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-red-50 border border-red-100 rounded-xl p-4">
              <div className="flex items-center gap-2 text-red-600 mb-2">
                ...
                <span className="font-semibold">Churn este mês</span>
              </div>
              <p className="text-3xl font-bold text-red-600">{metrics?.churn?.thisMonth ?? 0}%</p>
              <p className="text-sm text-red-500">Mês anterior: {metrics?.churn?.lastMonth ?? 0}%</p>
            </div>
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
              <div className="flex items-center gap-2 text-emerald-600 mb-2">
                <TrendingUp size={18} />
                <span className="font-semibold">Crescimento este mês</span>
              </div>
              <p className="text-3xl font-bold text-emerald-600">{metrics?.growth?.thisMonth ?? 0}%</p>
              <p className="text-sm text-emerald-500">Mês anterior: {metrics?.growth?.lastMonth ?? 0}%</p>
            </div>
          </div>
        </div>
      </div>
...
function PlanDistributionRow({ plan, count, total }: { plan: string; count: number; total: number }) {
  ...
  const planColors: Record<string, string> = {
    ...
    TIER_II: "blue",
    TIER_III: "purple",
    ENTERPRISE_CUSTOM: "amber",
  };
  const color = planColors[plan] || "gray";

  return (
    <div className="flex items-center gap-3">
      <div className={`w-8 h-8 rounded-lg flex items-center justify-center bg-${color}-100 text-${color}-600 text-sm font-bold`}>
        {plan.replace("TIER_", "T").replace("ENTERPRISE_CUSTOM", "ENT")}
      </div>
      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
        <div className={`h-full bg-${color}-500 rounded-full transition-all duration-500`} style={{ width: `${percentage}%` }} />
      </div>
